...
function adminLogout() {
  api('api/logout.php', { method: 'POST' }).then(function () { location.href = 'index.php'; });
}
...
function loadBookings() {
  api('api/booking.php').then(function (res) {
    ...
    list.innerHTML = '';
    var rows = res.data || [];
    if (!rows.length) { list.innerHTML = '<div style="opacity:.6;font-size:14px;">אין תורים קרובים</div>'; return; }
...
function cancelBooking(id) {
  if (!confirm('לבטל תור זה? תישלח הודעת ביטול ללקוח ולעסק.')) return;
...
function rescheduleBooking(id, date, time) {
  ...
  api('api/booking.php', { method: 'PUT', body: JSON.stringify({ id: id, action: 'reschedule', date: date, time: time }) }).then(function (res) {
    if (res.ok) { toast('התור עודכן'); loadBookings(); } else toast(res.data.error || 'שגיאה');
  });
}
...
function sendTestMail() {
  ...
  api('api/settings.php', { method: 'POST', body: JSON.stringify(payload) }).then(function () {
    ...
    if (res.ok) {
      resultEl.style.color = 'oklch(0.85 0.19 145)';
...
/* ── Push notifications (barber gets notified on new/changed bookings) ───
