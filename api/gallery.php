...
if ($method === 'GET') {
    $stmt = $pdo->prepare('SELECT id, filename, sort_order FROM gallery_media WHERE type=? ORDER BY sort_order ASC, id ASC');
    $stmt->execute([$type]);
    $rows = $stmt->fetchAll();
    $baseUrl = $type === 'image' ? IMAGES_URL : VIDEOS_URL;
...
if ($method === 'POST') {
    ...
    $countStmt = $pdo->prepare('SELECT COUNT(*) c FROM gallery_media WHERE type=?');
    ...
    if ($type === 'image') {
        ...
        imagejpeg($srcImg, IMAGES_DIR . $filename, IMAGE_JPEG_QUALITY);
        imagedestroy($srcImg);

        $stmt = $pdo->prepare('INSERT INTO gallery_media (type, filename, sort_order) VALUES (?, ?, ?)');
        $stmt->execute(['image', $filename, $count]);
        json_out(['ok' => true, 'filename' => $filename, 'url' => IMAGES_URL . $filename]);
    }
    ...
    if ($type === 'video') {
        ...
        if (!move_uploaded_file($file['tmp_name'], VIDEOS_DIR . $filename)) {
            json_out(['error' => 'שמירת הקובץ נכשלה'], 500);
        }
        $stmt = $pdo->prepare('INSERT INTO gallery_media (type, filename, sort_order) VALUES (?, ?, ?)');
        $stmt->execute(['video', $filename, $count]);
        json_out(['ok' => true, 'filename' => $filename, 'url' => VIDEOS_URL . $filename]);
    }
}
...
if ($method === 'DELETE') {
    ...
    $stmt = $pdo->prepare('SELECT * FROM gallery_media WHERE id=? AND type=?');
    $stmt->execute([$id, $type]);
    $row = $stmt->fetch();
    if ($row) {
        $dir = $type === 'image' ? IMAGES_DIR : VIDEOS_DIR;
