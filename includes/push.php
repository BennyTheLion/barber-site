<?php
/**
 * Sends a real browser push notification to every device the barber enabled push on.
 * Never throws — a broken push setup should never break the booking flow itself.
 */
function send_push_notifications($pdo, $title, $body, $url = null) {
    try {
        ...
        $row = $pdo->query("SELECT setting_value FROM settings WHERE setting_key='admin_notification_email'")->fetch();
